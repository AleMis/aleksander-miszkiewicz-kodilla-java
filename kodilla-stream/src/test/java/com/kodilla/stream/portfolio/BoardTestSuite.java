...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        //Then
        OptionalDouble sum = OptionalDouble.of(project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(x -> x.getTasks().stream())
                .map(Task::getCreated)
                .map(n -> ChronoUnit.DAYS.between(n, current))
                .reduce(0L, (res, curr) -> res = res + curr));

        OptionalDouble quantity = OptionalDouble.of(project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(x -> x.getTasks().stream())
                .map(Task::getCreated)
                .count());
